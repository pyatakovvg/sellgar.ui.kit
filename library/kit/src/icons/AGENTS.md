...
Отдельный icon slice библиотеки `@tiyn/kit`.
...
Иконки не являются частью core UI entrypoint `@tiyn/kit`.
...
- Добавление, удаление или переименование icon export является изменением public API `@tiyn/kit/icons`.
...
- Не создавать общий public import `@tiyn/kit/icons/generated`.
...
- Неправильный export ломает consumer imports из `@tiyn/kit/icons`.
...
- Нужна отдельная проверка tree-shaking стратегии для `@tiyn/kit/icons`.
...
- Проверять, что import из `@tiyn/kit/icons` резолвит изменённые иконки.
