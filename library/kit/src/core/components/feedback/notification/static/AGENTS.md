...
Путь: `src/core/components/feedback/notification/static`.

## Что это

`Static` — компонент в срезе `core/components`.

## Назначение

Static visual variant parent-компонента; не является самостоятельным public API.
...
- Public/local entrypoint: `src/core/components/feedback/notification/static/index.ts`.
- Source: `src/core/components/feedback/notification/static/static.tsx`.
...
- `export { Static } from './static.tsx';`
- Consumer import: прямой public import из `@tiyn/kit` не подтверждён; использовать через ближайший parent/component API.
- Exported/source names: `Static`.
- Props из `IProps`: `status`, `title`, `description`, `slot`.
- Локальных parts нет.

## Контракт изменения

- Не менять локальные exports (`export { Static } from './static.tsx';`) без оценки public API.
- Узел является scoped/internal для ближайшего родителя; не выводить его в public barrels без отдельного решения.
- Props contract: `status`, `title`, `description`, `slot`. Новые/изменённые props нужно отражать в story и документации.
...
- Локальные imports: `../../../../../../icons`, `../../../../primitives/content/typography`, `./default.module.scss`.
...
- Parent Storybook scenario: `../../clients/storybook/src/kit/symbols/notification/notification-default.stories.tsx`. Проверять изменение через parent-сценарий, потому что отдельной story для scoped/internal части нет.
