...
Путь: `src/core/components/feedback/animate`.

## Что это

`Animate` — компонент в срезе `core/components`.

## Назначение

Набор простых animation wrappers для визуальных состояний, например spin.
...
- Public/local entrypoint: `src/core/components/feedback/animate/index.ts`.
- Source: `src/core/components/feedback/animate/animate.tsx`.
