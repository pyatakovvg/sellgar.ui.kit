...
Путь: `src/core/components/controls/toggle/element`.
...
Внутренний visual/control element для Toggle: отвечает за switch-like часть внутри parent `Toggle`.
...
- Public/local entrypoint: `src/core/components/controls/toggle/element/index.ts`.
- Source: `src/core/components/controls/toggle/element/element.tsx`.
...
- Parent Storybook scenario: `../../clients/storybook/src/kit/symbols/toggle/toggle.stories.tsx`. Проверять изменение через parent-сценарий, потому что отдельной story для scoped/internal части нет.
