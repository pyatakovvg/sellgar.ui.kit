...
Путь: `src/core/components/controls/calendar/day-disabled`.

## Что это

`DayDisabled` — компонент в срезе `core/components/controls`.

## Назначение

Внутренний элемент календарной сетки для недоступного или внешнего дня месяца.
...
- Public/local entrypoint: `src/core/components/controls/calendar/day-disabled/index.ts`.
- Source: `src/core/components/controls/calendar/day-disabled/day-disabled.tsx`.
...
- Exported/source names: `DayDisabled`.
...
- Не менять локальные exports (`export { DayDisabled } from './day-disabled.tsx';`) без оценки public API.
