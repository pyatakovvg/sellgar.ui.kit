...
Путь: `src/core/components/controls/calendar/week-day`.

## Что это

`WeekDay` — компонент в срезе `core/components/controls`.

## Назначение

Внутренний элемент заголовка дня недели в Calendar.
...
- Public/local entrypoint: `src/core/components/controls/calendar/week-day/index.ts`.
- Source: `src/core/components/controls/calendar/week-day/week-day.tsx`.
...
- Exported/source names: `WeekDay`.
- Props из `IProps`: `isWeekday`.
- Локальных parts нет.

## Контракт изменения

- Не менять локальные exports (`export { WeekDay } from './week-day.tsx';`) без оценки public API.
- Узел является scoped/internal для ближайшего родителя; не выводить его в public barrels без отдельного решения.
- Props contract: `isWeekday`. Новые/изменённые props нужно отражать в story и документации.
