...
Путь: `src/core/components/controls/multi-select/option`.

## Что это

`Option` — компонент в срезе `core/components/controls`.
...
- Public/local entrypoint: `src/core/components/controls/multi-select/option/index.ts`.
- Source: `src/core/components/controls/multi-select/option/option.tsx`.
...
- Props из `IProps`: `title`, `checked`, `isIndeterminate`, `badge`, `disabled`.
- Локальных parts нет.
...
- Props contract: `title`, `checked`, `isIndeterminate`, `badge`, `disabled`. Новые/изменённые props нужно отражать в story и документации.
...
- Локальные imports: `../../../../components/content/typography`, `../../../../components/controls/checkbox`, `../../../../components/status/badge`, `./default.module.scss`.
