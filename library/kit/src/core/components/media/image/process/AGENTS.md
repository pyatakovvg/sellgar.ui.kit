...
Путь: `src/core/components/media/image/process`.

## Что это

`Process` — компонент в срезе `core/components`.

## Назначение

Внутреннее process/loading представление parent media component.
...
- Public/local entrypoint: `src/core/components/media/image/process/index.ts`.
- Source: `src/core/components/media/image/process/process.tsx`.
...
- Не менять локальные exports (`export { Process } from './process.tsx';`) без оценки public API.
...
- Локальные imports: `../../../../../../icons`, `../../../feedback/animate`, `./process.module.scss`.
