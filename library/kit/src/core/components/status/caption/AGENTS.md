...
Путь: `src/core/components/status/caption`.

## Что это

`Caption` — компонент в срезе `core/components`.

## Назначение

Статусный текстовый primitive для коротких пояснений и service copy.
...
- Public/local entrypoint: `src/core/components/status/caption/index.ts`.
- Source: `src/core/components/status/caption/caption.tsx`.
...
- Storybook scenario: `../../clients/storybook/src/kit/symbols/caption/caption.stories.tsx`.
