...
Путь: `src/core/components/navigation/tab-menu/tab/segmented`.

## Что это

`SegmentedTab` — компонент в срезе `core/components`.

## Назначение

Внутренний visual renderer для одной вкладки внутри `TabMenu.Segmented`.
...
- Public/local entrypoint: `src/core/components/navigation/tab-menu/tab/segmented/index.ts`.
- Source: `src/core/components/navigation/tab-menu/tab/segmented/segmented.tsx`.
...
- Exported/source names: `SegmentedTab`.
- Props из `IProps`: `size`, `shape`, `isActive`, `title`, `name`, `id`, `ariaControls`, `leadIcon`, `tailIcon`, `badge`, `disabled`, `onClick`.
- Локальных parts нет.

## Контракт изменения

- Не менять локальные exports (`export { SegmentedTab } from './segmented.tsx';`) без оценки parent API.
