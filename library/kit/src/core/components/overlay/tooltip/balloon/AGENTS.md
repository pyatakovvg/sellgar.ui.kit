...
Путь: `src/core/components/overlay/tooltip/balloon`.

## Что это

`Balloon` — локальная часть `ToolTip` в срезе `core/components/overlay`.

## Назначение

Внутренняя визуальная часть Tooltip: отображает floating balloon content.
...
- Public/local entrypoint: `src/core/components/overlay/tooltip/balloon/index.ts`.
- Source: `src/core/components/overlay/tooltip/balloon/balloon.tsx`.
- Собственных child nodes нет.
- Использует memoized derivation/callbacks; зависимости hooks должны оставаться точными.

## Public API и локальные файлы

- `export { Balloon } from './balloon.tsx';`
- Consumer import: прямой public import из `@sellgar/kit` не подтверждён; использовать через ближайший parent/component API.
- Exported/source names: `Balloon`.
- Props из `IProps`: `size`.
- Локальных parts нет.

## Контракт изменения

- Не менять локальные exports (`export { Balloon } from './balloon.tsx';`) без оценки public API.
...
- Parent Storybook scenario: `../../clients/storybook/src/kit/symbols/tooltip/tooltip.stories.tsx`. Проверять изменение через parent-сценарий, потому что отдельной story для scoped/internal части нет.
