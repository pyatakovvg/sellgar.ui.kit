...
Путь: `src/core/components/overlay/drawer`.

## Что это

`Drawer` — controlled overlay interaction в срезе `core/components/overlay`.

## Назначение

Overlay interaction для боковой панели поверх страницы на базе OverlayDialog и Scrollbar.
...
- Делегирует open/close/focus/outside-press lifecycle в `OverlayDialog`; сам `Drawer` отвечает за drawer layout, scroll container и public close API.

## Как реализован

- Public/local entrypoint: `src/core/components/overlay/drawer/index.ts`.
- Source: `src/core/components/overlay/drawer/drawer.tsx`.
...
- Controlled `open` приходит от consumer; внутреннего open state в `Drawer` нет.
...
- `Drawer.Close` проксирует `OverlayDialog.Close`; изменение этого static member ломает compound API.
...
- Работает с refs; изменение DOM target или ref type может быть breaking change.
- Имеет callback props; порядок вызовов и disabled guards являются частью поведения.
- `OverlayDialog.Content` передаёт `content.ref` в `Scrollbar`; изменение ref target влияет на floating/focus behavior.

## Зависимости

- Внешние runtime imports: `react`.
- Локальные imports: `../../../systems/floating/internals/overlay-dialog`, `../../..`, `./default.module.scss`.
...
- Главный риск — незаметно изменить close lifecycle, scroll container или DOM target, на который завязан Floating UI.
...
- Нет защиты от повторных close requests во время async `onRequestClose`.
- Storybook scenario автоматически не найден; для drawer нужен отдельный сценарий проверки.
